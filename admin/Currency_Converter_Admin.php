<?php
/**
 * Currency_Converter_Admin Class
 *
...
class Currency_Converter_Admin {
	...
	public function register_settings_page() {
		...
		add_settings_section(
			...
			esc_html_e( 'Exchangerates.io', 'currency-converter' ),
	 ...
	 * @return void
	 */
	public function currency_converter_main_callback( array $args ): void {
		?>
	 ...
	 * @return void
	 */
	public function currency_converter_exchangeratesapi_callback( array $args ): void {
		?>
	 ...
	 * @return void
	 */
	public function currency_converter_fixer_callback( array $args ): void {
		?>
	...
	public function currency_converter_api_provider( $args ) {
		...
		<input
				...
				name="<?php echo esc_attr( self::CURRENCY_CONVERTER_OPTIONS_META_KEY ); ?>[<?php echo esc_attr( $args['label_for'] ); ?>]"
				value="exchangeratesapi.io"
				<?php echo isset( $options[ $args['label_for'] ] ) && 'exchangeratesapi.io' === $options[ $args['label_for'] ] ? ' checked="checked"' : ''; ?>
		/>
		...
		<input
				...
				name="<?php echo esc_attr( self::CURRENCY_CONVERTER_OPTIONS_META_KEY ); ?>[<?php echo esc_attr( $args['label_for'] ); ?>]"
				value="fixer.io"
				<?php echo isset( $options[ $args['label_for'] ] ) && 'fixer.io' === $options[ $args['label_for'] ] ? ' checked="checked"' : ''; ?>
		/>
		<label for="<?php echo esc_attr( $args['label_for'] ); ?>_fixer">fixer.io</label> <a href="https://fixer.io/" target="_blank"><?php echo esc_html_e( 'register here', 'currency-converter' ); ?></a><br>
		<?php
	}
	 ...
	 *
	 * @param array $args atrributes
	 * @return void
	 */
	public function currency_converter_base_currency( array $args ): void {
		$options = get_option( self::CURRENCY_CONVERTER_OPTIONS_META_KEY );
		?>
		<input
				type="radio"
				...
				name="<?php echo esc_attr( self::CURRENCY_CONVERTER_OPTIONS_META_KEY ); ?>[<?php echo esc_attr( $args['label_for'] ); ?>]"
				value="EUR"
				<?php echo( isset( $options[ $args['label_for'] ] ) && 'EUR' === $options[ $args['label_for'] ] ) || empty( $options[ $args['label_for'] ] ) ? ' checked="checked"' : ''; ?>
		/>
		...
		<input
				...
				name="<?php echo esc_attr( self::CURRENCY_CONVERTER_OPTIONS_META_KEY ); ?>[<?php echo esc_attr( $args['label_for'] ); ?>]"
				value="USD"
				<?php echo isset( $options[ $args['label_for'] ] ) && 'USD' === $options[ $args['label_for'] ] ? ' checked="checked"' : ''; ?>
		/>
		...
		<input
				...
				name="<?php echo esc_attr( self::CURRENCY_CONVERTER_OPTIONS_META_KEY ); ?>[<?php echo esc_attr( $args['label_for'] ); ?>]"
				value="GBP"
				<?php echo isset( $options[ $args['label_for'] ] ) && 'GBP' === $options[ $args['label_for'] ] ? ' checked="checked"' : ''; ?>
		/>
		<label for="<?php echo esc_attr( $args['label_for'] ); ?>_gbp"><?php echo esc_html_e( 'British Pound', 'currency-converter' ); ?></label><br>
		<p class="description" style="color: red"><?php echo esc_html_e( 'Warning! If you are using free plan choose EURO! Free plan do not allow any other base currency', 'currency-converter' ); ?> </p>
		<?php
	}
	 ...
	 * @return void
	 */
	public function currency_converter_exchangeratesapi_key( array $args ): void {
	 ...
	 * @return void
	 */
	public function currency_converter_fixer_key( array $args ): void {
		// Get the value of the setting we've registered with register_setting()
		$options = get_option( self::CURRENCY_CONVERTER_OPTIONS_META_KEY );
		?>
		<input
	 ...
	 * @return void
	 */
	public function currency_converter_options_page(): void {
		add_menu_page(
			'Currency Converter',
			'Currency Converter',
			'manage_options',
	...
	public function currency_converter_options_page_html() {
		...
		if ( isset( $_GET['settings-updated'] ) ) {
			...
			if ( $currency_converter_data->is_inicialized() ) {
				$currency_converter_data->force_refresh_data();
				$response = $currency_converter_data->validate_connection();
				if ( $response ) {
